/** Headless-safe npm launcher for the VideoBuddy Electron executable. */
...
Launch VideoBuddy with the selected Web-capable profile.
...
export function defaultDesktopUserDataDirectory(
  ...
  if (platform === 'win32') {
    ...
    if (appData === undefined || appData.length === 0) {
      throw new Error('APPDATA is unavailable; cannot locate VideoBuddy diagnostics')
    }
    return path.join(appData, 'VideoBuddy')
  }
  if (platform === 'darwin') return path.join(homeDirectory, 'Library', 'Application Support', 'VideoBuddy')
  const config = environment.XDG_CONFIG_HOME
  return path.join(config === undefined || config.length === 0 ? path.join(homeDirectory, '.config') : config, 'VideoBuddy')
}
...
async function launchElectron(): Promise<number> {
  ...
  try {
    ...
    process.stderr.write(
      ...
      + 'Or use the packaged VideoBuddy application.\n',
    )
    return 1
  }
