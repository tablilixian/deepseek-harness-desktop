...
画布式 AI 视频创作工作流插件:左栏项目列表,中间无限画布(顶部固定工具栏 + 底部分镜时间线),右栏仅为官方对话区。图层列表作为可开关的悬浮面板叠在画布右上角,工具栏集中了缩放 ±/适配/重置、小地图显隐、图层显隐和所有编辑操作。agent 在对话中编排分镜、角色定妆、场景概念、视频片段与合成,节点实时落在画布上,可打断、改提示、单节点重试。
