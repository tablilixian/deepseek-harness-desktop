...
export class ProjectRegistry {
    ...
    async removeProject(projectId) {
        ...
        if (!dir.startsWith(this.projectsDir + sep))
     ...
     */
    async updateWorkflow(projectId, patch) {
        const projects = [...await this.list()];
        const index = projects.findIndex((entry) => entry.id === projectId);
        if (index === -1)
            throw new Error(`项目不存在: ${projectId}`);
     ...
     */
    async setPendingQuestion(projectId, question) {
        const projects = [...await this.list()];
        const index = projects.findIndex((entry) => entry.id === projectId);
        if (index === -1)
            throw new Error(`项目不存在: ${projectId}`);
     ...
     */
    async answerPendingQuestion(projectId, value) {
        const projects = [...await this.list()];
        const index = projects.findIndex((entry) => entry.id === projectId);
        if (index === -1)
            throw new Error(`项目不存在: ${projectId}`);
