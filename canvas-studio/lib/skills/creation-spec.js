...
export const CREATION_SKILL_DESCRIPTION = 'Canvas Studio 画布视频创作规范：点选式需求澄清（ask_user_choice）、分镜表审批门禁、镜头参数词汇、MiniMax H3 视频提示词规范与十一个媒体工具的标准串联流程。凡涉及生成图片/视频、分镜规划、AI 短片或漫剧创作时使用。';
...
| 预设 | 适用 | 流程差异 / 关键约束 |
| --- | --- | --- |
| 极简产品广告 | 电商 / 新品发布极简风广告短片 | 先确认产品与变体→提炼卖点→写简洁英文广告文案→分镜与音乐节拍同步；禁用 KOC 口播、普通剪辑、屏幕演示。H3 提示词走电影感极简构图、少元素、干净背景。 |
| 3D 动画短片 | 完整风格化 3D 动画 | 走标准动画流程：简报→大纲→角色/环境设定卡→镜头规划→故事板→逐镜→合成配乐；强调角色一致、场景连续、节奏精准。定妆锚点用 3D 角色设定图。 |
| 纸艺定格讲解 | 科学/教育/通识的手工纸艺讲解 | 纸艺角色 + 分层立体布景 + 道具；转场用纸张翻折/位移；H3 提示词强调 papercraft、手工质感、定格停顿。 |
| 品牌宣传 | Logo/产品图/官网截图→品牌短片 | 选叙事方向→精确节奏点→生成素材(图/视频/旁白/音乐)→剪辑→明确 CTA；突出功能与使用场景。 |
| MV 字幕 | 与节拍同步的歌词字幕 MV | 先分析歌曲→歌词排版与节拍对齐→镜头拼接指导；字幕走 write_script 文案节点（仅展示，不烧录）。 |
| 合作游戏开场 | 双人合作游戏菜单/开场动画 | 收集两玩家名+游戏标题+视觉风格→先出「确认图」锁视觉（对应第 5 步定妆锚点）→用户确认后再生成开场视频；含玩家卡片与菜单交互动效。 |
| 纸拼贴讲解 | 半色调纸拼贴动画阐述概念 | 视觉隐喻→分镜→半色调拼贴静帧→纸张运动+拟声音效定格；默认保留纸张触碰拟声（写进 overall_soundscape），不加 BGM/旁白/字幕，除非用户要求。 |
| 手绘实景融合 | 手绘发光动画+实拍空间融合超现实 | 设计连续变形/逃脱路径+手持追拍→写一段 15s、16:9 的 H3 提示词；前 3 秒必须与真实手/物体清晰接触并同体变形，镜头总慢半拍追赶。 |

**能力边界（必须如实告知用户）**：
- 字幕 / 对白 / 广告词 / 旁白文案：一律走 write_script 落到「文案」节点（成片详情展示），**当前不烧录进画面**；如用户要硬字幕，需自备含字幕的素材或等后续工具支持。
- 背景音乐：用用户提供的 BGM 文件节点（compose_video 的 bgmNodeId 混入）；**当前无音乐生成工具**，不要承诺自动作曲。
- 旁白 TTS：当前无语音合成工具，旁白文案由 write_script 产出，音频需用户自备或仅作文案。
